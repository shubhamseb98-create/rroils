...
if (isset($_POST['submit'])) {
	...
	$bannerType = isset($_POST['banner_type']) ? mysqli_real_escape_string($conn, $_POST['banner_type']) : 'image';
	...
	$bvideo = '';
	$bvideos = $_FILES['bvideo']['name'];
	if ($bannerType == 'video' && $bvideos != '') {
		$bvideo = time() . "_" . $bvideos;
		move_uploaded_file($_FILES["bvideo"]["tmp_name"], "../uploads/banner/" . $bvideo);
	}
	
	$query = mysqli_query($conn, "INSERT INTO `tbl_banner`(`bnr_title`,`bnr_subtitle`,`bnr_desc`, `bnr_sort`, `bnr_url`, `bnr_image`, `bnr_status`, `bnr_type`, `bnr_video`) VALUES ('$title','$subtitle','$desc','$position','$link','$bimage','$status','$bannerType','$bvideo')");
	if ($query == true) {
						...
						<form role="form" method="POST" enctype="multipart/form-data">
							<div class="box-body">
								...
								<div class="form-group">
									...
									<input type="radio" value="image" id="bannerTypeImage" name="banner_type" checked>
									<label for="bannerTypeImage">Image Banner</label>
									<input type="radio" value="video" id="bannerTypeVideo" name="banner_type">
									<label for="bannerTypeVideo">Video Banner</label>
								</div>

								<div class="form-group" id="imageUploadBlock">
								...
								<div class="form-group" id="videoUploadBlock" style="display:none;">
