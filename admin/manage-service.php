...
if(isset($_POST['Dectivate']) && isset($_POST['bb']))
{
  ...
  foreach($bb as $act)
  {
	  mysqli_query($conn,"update tbl_service  set status='0' where id='$act'");
  }
}
...
if(isset($_POST['Activate']) && isset($_POST['bb']))
{
  ...
  foreach($bb as $act)
  {
	  mysqli_query($conn,"update tbl_service  set status='1' where id='$act'");
  }
}
...
if(isset($_POST['Delete']) && isset($_POST['bb']))
{
    $bb = $_POST['bb'];
      foreach($bb as $act)
  {
	 mysqli_query($conn,"DELETE FROM tbl_service WHERE id=$act");
  }
}

		
$mqry="select * from tbl_service  ";
...
<body>
                     ...
                     <form name="myform" method="post" action=""> 
						...
						<div class="alert alert-secondary fade show">
							...
							<div class="btn-group btn-group-justified">
                              <a href="add-service.php" class="btn btn-default active"><i class="fa fa-plus"></i> Add New Product</a>
							...
							<table id="data-table-responsive" class="table table-striped table-bordered">
								<thead>
									<tr>
										...
										<!--<th class="text-nowrap">Benifits</th>-->
			                    ...
			                    ?>
									<tr class="odd gradeX">
									   ...
									   <td width="10%" class="with-img"><?php if($web['image']==''){ ?><img src="../uploads/no_img.jpg" class="img-rounded height-50" /><?php }else{ ?><img src="../uploads/product/<?php echo $web['image']; ?>" class="img-rounded height-60" /><?php } ?></td>
                                       <td style="font-weight:700; color:#000;"><?= $web['name'];?></td>
									  <!--<td style="font-weight:700; color:#000;"><a href="manage-videos.php?pid=<?php echo $web['id'];?>"><?= $web['name'];?> Benifits</a></td>-->
										<td>
                                          <div class="switcher">
										...
										<td>
                                          <a href="edit-service.php?bid=<?php echo $web['id'];?>" class='label label-sm label-primary' title="Edit"><i class="fa fa-edit"></i> Edit</a>
                                        </td>
                                        <td>
                                          <a href="delete/service.php?bid=<?php echo $web['id'];?>" onClick="if(confirm('Are You Sure Want To Delete This Record')){ return true;} else { return false; }" class='label label-sm label-danger'><i class="fa fa-trash"></i> Delete</a>
                                         </td>
                                        <td width="1%">
    ...
    <script>
	  function updateId(id)
	  {
		  ...
		  xmlhttp.open("GET", "status/service.php?id=" +id, true);
		  xmlhttp.send();
	  }
   </script>
    <!----------------Check Box-----------------------> 
